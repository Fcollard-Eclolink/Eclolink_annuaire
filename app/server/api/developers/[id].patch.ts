interface DeveloperPatch {
  first_name?: string
  last_name ?: string
  agency    ?: string | null
  job_title ?: string | null
}

export default defineEventHandler(async (event): Promise<Developer> => {
  const id = getRouterParam(event, 'id')
  if (!id) throw createError({ statusCode: 400, message: 'ID requis' })

  const body = await readBody<DeveloperPatch>(event)
  if (body.first_name !== undefined && !body.first_name.trim()) {
    throw createError({ statusCode: 400, message: 'Le prénom est requis' })
  }
  if (body.last_name !== undefined && !body.last_name.trim()) {
    throw createError({ statusCode: 400, message: 'Le nom est requis' })
  }

  const res = await sbFetch(event, `/rest/v1/eclolink_developers?id=eq.${encodeURIComponent(id)}`, {
    method : 'PATCH',
    body   : JSON.stringify(body),
    headers: { Prefer: 'return=representation' },
  })

  const rows = (await res.json()) as Developer[]
  if (!rows[0]) throw createError({ statusCode: 404, message: 'Développeur introuvable' })
  return rows[0]
})
